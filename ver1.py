...
def tabu_search(cur_vehs: list, iter_max: int = 200, time_limit: int = 150, p_l = 1, p_t = 1, p_f = 0.0001)\
    ...
    while iteration <= iter_max and searching_time < time_limit:
        ...
        for c in range(len(nodes)):
            # if c in cur_vehs[nodes[c].belong_veh].o_additional.keys():
            #     c = cur_vehs[nodes[c].belong_veh].o_additional[c]
            # print(c)
            # print(nodes[c].belong_veh)
            # print(cur_vehs[nodes[c].belong_veh].v_id)
            # print(cur_vehs[nodes[c].belong_veh].o_additional.keys())

            if nodes[c].c_id in cur_vehs[nodes[c].belong_veh].o_additional.keys():
                continue
                # ori_point = cur_vehs[nodes[c].belong_veh].o_additional[c]
                # nodes[ori_point].demand -= 1
                # del cur_vehs[nodes[c].belong_veh].o_additional[c]
            ...
            for k in range(len(cur_vehs)):
                #if len(cur_vehs[k].route) >= 1 and nodes[c].belong_veh != k:
                if nodes[c].belong_veh != k:
                    # if cur_vehs[k].cap - cur_vehs[k].load >= 1:
                    #     rangeToInsert = len(cur_vehs[k].route) + 1
                    # else:
                    #     rangeToInsert = len(cur_vehs[k].route)
                    for p in range(len(cur_vehs[k].route) + 1):
                        ...
                        if nodes[c] in cur_vehs[nodes[c].belong_veh].o_additional.values():
                            ...
                            # print(nodes[c].belong_veh)
                            # print(cur_vehs[nodes[c].belong_veh].route)
        ...
        if neighbor_best_obj < global_best_obj and status:
            ...
            global_best_vehs = copy.deepcopy(cur_vehs)
            tabu[best_cus][nodes[best_cus].belong_veh] = tabu_tenure
